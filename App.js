...
export default function App() {
  ...
  const [viewPopupAnimation, setViewPopupAnimation] = useState(new Animated.Value(0));
  const [createPopup, setCreatePopup] = useState(false);
  const [createPopupAnimation, setCreatePopupAnimation] = useState(new Animated.Value(0));
  const [selectedTaskIndex, setSelectedTaskIndex] = useState(0);

  useEffect(() => {
    async function fetchData() {
      setTasks(await getValueFor('tasks'));
